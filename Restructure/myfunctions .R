...
{# samplesheet <- read_samplesheet(dir_in=input)
  ...
  if (sum(is.na(group)) > 0) {
    ...
    if (ncol(count_data) != 6) {
      ...
      count_data = mutate(count_data,ORF = str_sub(ORF, end = -5))
      return(count_data)
    }
  }
}
